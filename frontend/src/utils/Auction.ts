...
export async function getAuctionData(wallet: WalletState, auctionAddr: string): Promise<string[]> {
    ...
    const bider_bid = ethers.formatEther(await auction.fundsByBidder(signer.address))
    const status = (await auction.auctionStatus()).toString()
    const highestBidder = (await auction.highestBidder())

    return [getCidFromDataUri(tokenUriData), bid, bidder, seller, tokenAddr, increment, bider_bid, status, highestBidder]
}
