...
//import {AuctionAbstract} from "./AuctionAbstract.sol";
...
contract Auction is IAuction, ReentrancyGuard {
    ...
    AuctionItems public immutable auctionItems;
    // auction items are LSP8 ERC721 NFT
    ...
    constructor(
        address _bidToken,
        address payable _auctionItems,
        uint256 _duration,
        AuctionConfig memory _config // LSP9Vault(_config.owner)
    ) {
        require(
        ...
        auctionItems = AuctionItems(_auctionItems);
        ...
        require(
            auctionItems.tokenOwnerOf(_config.itemTokenId) == auctionFactory,
        ...
        returns (bool success)
    {
        ...
        require(
            auctionItems.tokenOwnerOf(config.itemTokenId) == address(this),
        ...
        nonReentrant
    {
        ...
        if (bal > 0) {
            ...
            require(
                success,
                "[Auction] Rescue failed to withdraw Auction tokens."
            );
        }
        if (auctionItems.tokenOwnerOf(config.itemTokenId) == address(this)) {
    ...
    function _transferItemTo(address to) private {
        ...
        AuctionItems(auctionItems).safeTransferFrom(
            address(this),
            to,
            config.itemTokenId
        );
